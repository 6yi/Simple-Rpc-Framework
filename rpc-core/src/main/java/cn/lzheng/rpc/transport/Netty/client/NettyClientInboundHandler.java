...
/**
 * @ClassName NettyClientInboundHandler
 * @Author 6yi
 * @Date 2021/5/7 0:38
 * @Version 1.0
 * @Description:
...
public class NettyClientInboundHandler extends SimpleChannelInboundHandler<RpcResponse> {
    ...
    public NettyClientInboundHandler(){
        cacheRequest=CacheRequest.getCacheRequest();
    }
    @Override
    protected void channelRead0(ChannelHandlerContext ctx, RpcResponse msg) throws Exception {
        try {
            logger.info(String.format("客户端接收到消息: %s", msg));
            cacheRequest.complete(msg);
        } finally {
